...
async function main(params) {
  ...
  try {
    ...
    const requiredParams = [ 'SFMC_HEROES_CLIENT_ID' , 'SFMC_HEROES_CLIENT_SECRET', 'SFMC_HEROES_AUTH_URI', 'SFMC_HEROES_ACCOUNT_ID']
    ...
    const authUrl = new URL('/v2/token', params.SFMC_HEROES_AUTH_URI);
    const token = await getToken(
      params.SFMC_HEROES_CLIENT_ID,
      params.SFMC_HEROES_CLIENT_SECRET,
      params.SFMC_HEROES_ACCOUNT_ID,
      params.SFMC_HEROES_SCOPES || false,
      authUrl.toString()
    );
    ...
    return {SFMC_HEROES_TOKEN: token, ...params}
